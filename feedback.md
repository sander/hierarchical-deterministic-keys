...
|WTE_15–16|None.|N/A|
|WTE_17|Modify: “During PID <del>or attestation</del> issuance, a <del>WSCA</del> <ins>Wallet Instance</ins> SHALL prove possession of the private key corresponding to a <del>WTE</del> <ins>ITE</ins> public key on request of a PID Provider <del>or Attestation Provider via the Wallet Instance</del>, for example by signing a challenge with that private key.”|With HDK, there is no need for roles other than PID Providers to learn wallet metadata. Therefore, there is no reason to disclose an ITE.|
|WTE_18|Modify: “During PID or attestation issuance, a <del>WSCA</del> <ins>Wallet Instance</ins> SHOULD generate a proof of association for two or more public keys, if and only if <ins>these are provided for issuance and</ins> the corresponding private keys are protected by the same WSCA and the WSCA has internally registered an association between these private keys.”|In HDK, only one public key is provided for issuance. Therefore a proof of association should not be applicable.|
|WTE_19|Modify: “During PID or attestation issuance, the PID Provider or Attestation Provider SHALL verify that:<br>• The <del>WSCA</del> <ins>Wallet Instance</ins> described in the <del>WTE</del> <ins>ITE, if any,</ins> received from the Wallet Instance has proven possession of the private key corresponding to the public key in the <del>WTE</del> <ins>ITE</ins> (see WTE_17),<br>•The <del>WSCA</del> <ins>Wallet Instance</ins> has proven possession of the new PID or attestation private key (see WTE_14)<br>In addition, the PID Provider or Attestation Provider SHOULD verify that:<br>• The WSCA <del>has proven association (see WTE_18) between</del> <ins>protects with the same access controls</ins> the new PID or attestation public key and the public key requested by the PID Provider or Attestation Provider according to WTE_15 or WTE_16.”|In HDK, only PID Providers require ITE. Also, the PID Provider or Attestation Provider can apply HDK instead of a proof of association to verify equivalent WSCA protection between two keys.|
|WTE_20|Modify: “During PID <del>or attestation</del> issuance, a Wallet Instance SHALL provide the PID Provider <del>or Attestation Provider</del> with the <del>WTE</del> <ins>ITE</ins> describing the properties of the WSCA that generated the new PID <del>or attestation</del> private key and protects it.”|In HDK, only PID Providers require ITE.|
|WTE_21–22|None.|N/A|
|WTE_23|Modify: “The common OpenID4VCI protocol defined in requirement ISSU_01 SHALL enable a Wallet Instance to transfer a<ins>n ITE</ins> <del> WTE</del> to a PID Provider <del>or Attestation Provider</del>.”|In HDK, only PID Providers require ITE.|
|WTE_24|Modify: “A Wallet Instance SHALL release a WTE only to a <del>PID Provider or Attestation Provider</del> <ins>Wallet Provider</ins>, and not to a Relying Party or any other party.”<br><br>Add: “<ins>A Wallet Instance SHALL release a ITE only to a PID Provider, and not to a Relying Party or any other party.</ins>”|In HDK, only Wallet Providers require WTE and only PID Providers require ITE.|
|WTE_25–26|None.|N/A|
|WTE_27|Add: “The common OpenID4VCI protocol SHALL enable a PID Provider or Attestation Provider to indicate in the Token Response:<br><ins>• in the case of asynchronous remote key generation, data enabling the Wallet Instance to prove possession of the private key associated with the new PID or attestation key</ins>”|This is essential to enable issuance of multiple batches in HDK.|
|WTE_28–30|None.|N/A|
|WTE_31|TBD|TBD|
|WTE_32|None.|N/A|
...
|--|--|--|
|ACP_01–03|None.|N/A|
|ACP_04|Drop or modify: “If (as a result of ACP_03) a Wallet Instance determines it must release multiple attestations to a Relying Party in a combined presentation of attributes, it <del>SHALL</del><ins>MAY</ins> request a proof of association between the public keys of these attestations from the WSC<ins>A</ins>.”|Proof of association could introduce disproportional complications. See [Proof of association](keys.md#proofs-of-association).|
|ACP_05|Drop.|It is up to the User whether to share a proof of association with another party.|
|ACP_06–08|None.|N/A|
